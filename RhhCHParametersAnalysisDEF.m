%Calculation of Rhh and RTx for Final Scenarios
%OPTIONS:
...
for nn=1:length(N_vec)
    for pp=1:length(P_vec)
        for d=1:length(d_vec)
            cd RhhDEFSistemas\'GAUSS';%\Grafica1;      %Hasta Gauss excepto cuando sea con RADIO 50 que es para la Gráfica 1!!!!
                                                     %PDF: GaussCyl suggested by Molisch, or Gauss by COST259
            name2 = strcat(num2str(N_vec(nn)),'x',num2str(P_vec(pp)),'Physical',num2str(d_vec(d)),num2str(d_vec(d)),...
                'SUMacrocellBUscatt',num2str(50),'r',num2str(100),'LoSsiSSsiDSsi.mat');  
                              %it can be LoSsiSSnoDSsi, LoSnoSSnoDSsi, LoSsiSSsiDSsi, LoSnoSSsiDSsi
            load(name2);
            cd ..; cd ..; %cd ..;    %El tercer cd sólo para Grafica 1!!!
            ...
            for bs=1:Escenario.num_BS
                for ms=1:Escenario.N_d{bs}
                    for lt=1:H_Total(1).Limite_Taps
                        ...
                        Rhh.EigValues{Escenario.deseados{bs}(ms)}(lt) = s(1,1);
                        Rhh.EigVectors{Escenario.deseados{bs}(ms)}(:,lt) = u(:,1);
                        ...
                        Rtx.EigValues{Escenario.deseados{bs}(ms)}(lt) = stx(1,1);
                        Rtx.EigVectors{Escenario.deseados{bs}(ms)}(:,lt) = utx(:,1);
                        Rtx.Rtx{Escenario.deseados{bs}(ms),lt} = Rtx_temp{Escenario.deseados{bs}(ms),lt};
                    end
                end
            end
            name = ['RhhGauss',strcat(num2str(N_vec(nn))),'x',num2str(P_vec(pp)),num2str(d_vec(d)),num2str(d_vec(d)),'LoSsiSSsiDSsi','.mat'];
            save(name,'Rhh','Rtx');
        end
    end
end